...
percentRUNX2 <- lapply(res, cofactors, FUN = function(x, cofactors){
    (sum(ifelse(x$padj<0.05 & row.names(x) %in% cofactors, 1, 0), na.rm = TRUE)/length(cofactors))*100
  })
percentRUNX2 <- as.data.frame(percentRUNX2)
rownames(percentRUNX2) = "RUNX2"
percentRUNX2
saveRDS(percentRUNX2, "../BM_ATACr/percentRUNX2")
```
# Computing Sensitivity and FDR for StabSel
...
agmS <- data.frame(method = "StabSel",
                   ...
                   FDR = mean(agS$FDR))
saveRDS(agmS, "../BM_runATAC/FDR_Sensitivity_StabSel")
```