...
percentESR1 <- lapply(res, cofactors, FUN = function(x, cofactors){
    (sum(ifelse(x$padj<0.05 & row.names(x) %in% cofactors, 1, 0), na.rm = TRUE)/length(cofactors))*100
  })
percentESR1 <- as.data.frame(percentESR1)
rownames(percentESR1) = "ESR1"
percentESR1
saveRDS(percentESR1, "../BM_ATACr/percentESR1")
```

# Second Benchmark to also include ESR2 interactors
...
Benchmark <- BenchATAC(rawpaths, 
                    ...
                    motifpath, 
                    species)
Benchmark$padj <- subset(Benchmark$padj, !duplicated(method)) # get rid of ESR2 padj so plotting script does not crash
Benchmark$true_rank_table <- lapply(Benchmark$true_rank_table, function(mat) mat[mat[, "name"] == "ESR1", , drop = FALSE]) # get rid of ESR2 rank so sensitivity works
