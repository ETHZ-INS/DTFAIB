...
```{r}
BMG <- readRDS("../BM_runATAC/BMGATA1.rds")
BMG$AUC_plots
VAUC <- BMG$AUC_plots$VIPER
msVAUC <- BMG$AUC_plots$msVIPER
CV <- BMG$AUC_plots$CV

VAUC <- VAUC+ labs(x = "Rank", y = "Proportion of identified known network members")
            + theme(axis.text = element_text(size = 20),
                    axis.title = element_text(size = 20))
msVAUC <- msVAUC+ labs(x = "Rank", y = "Proportion of identified known network members")
            + theme(axis.text = element_text(size = 20),
                    axis.title = element_text(size = 20))
CV <- CV+ labs(x = "Rank", y = "Proportion of identified known network members")
            + theme(axis.text = element_text(size = 20),
                    axis.title = element_text(size = 20))

# extract data from the individual plots

data_VAUC <- layer_data(VAUC)
data_msVAUC <- layer_data(msVAUC)
data_CV <- layer_data(CV)
data_CV$Optimum <- BMG$res$CV$optimalScore[1:100] # Add the optimum curve

# Create a new ggplot with all the curves
combined_plot <- ggplot() +
  geom_line(data = data_VAUC, aes(x, y, color = "VIPER")) +
  geom_point(data = data_VAUC, aes(x, y, color = "VIPER")) +
  geom_line(data = data_msVAUC, aes(x, y, color = "msVIPER")) +
  geom_point(data = data_msVAUC, aes(x, y, color = "msVIPER")) +
  geom_line(data = data_CV, aes(x, y, color = "chromVAR")) + 
  geom_point(data = data_CV, aes(x, y, color = "chromVAR")) +
  geom_line(data = data_CV, aes(x, Optimum, color = "Optimum")) +
  geom_point(data = data_CV, aes(x, Optimum, color = "Optimum")) +
  ylab("Proportion of listed known network members")+
  xlab("Rank")+
  scale_color_manual(values = c(VIPER = "blue", msVIPER = "cyan", chromVAR = "orange", Optimum = "red"))+
...
ggsave("../BM_runATAC/Plots/GATA1AUCs.png", plot = combined_plot, width = width, height = height, dpi = dpi)
```
...
percentGATA1 <- lapply(res, cofactors, FUN = function(x, cofactors){
    (sum(ifelse(x$padj<0.05 & row.names(x) %in% cofactors, 1, 0), na.rm = TRUE)/length(cofactors))*100
  })
percentGATA1 <- as.data.frame(percentGATA1)
rownames(percentGATA1) = "GATA1"
percentGATA1
saveRDS(percentGATA1, "../BM_ATACr/percentGATA1")
```
...
```{r}
agS <- data.frame(sensitivity=sensitivity,
            FDR=FDR)
```

```{r}
agmS <- data.frame(method = "StabSel",
                   sensitivity = mean(agS$sensitivity),
                   FDR = mean(agS$FDR))
```
