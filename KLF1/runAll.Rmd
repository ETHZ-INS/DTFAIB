...
percentKLF1 <- lapply(res, cofactors, FUN = function(x, cofactors){
    (sum(ifelse(x$padj<0.05 & row.names(x) %in% cofactors, 1, 0), na.rm = TRUE)/length(cofactors))*100
  })
percentKLF1 <- as.data.frame(percentKLF1)
rownames(percentKLF1) = "KLF1"
percentKLF1
saveRDS(percentKLF1, "../BM_ATACr/percentKLF1")
```

```{r}
KBM <- readRDS("../BM_runATAC/BMKLF1.rds")

runtime_list <- KBM$runtime

new_names <- c("CV", "CVadj", "decouple", "FGSEA", "ML", "StabSel", "msVIPER", "regreg", "ULM", "VIPER")
names(runtime_list) <- new_names

KET <- sapply(runtime_list, function(x) as.numeric(x["elapsed"]/3600))

barplot(KET, col = "blue", main = "Elapsed Runtime for Each Method",
        xlab = "Methods", ylab = "Runtime (hours)", cex.names = 0.6)
```

```{r}
KET <- as.data.frame(KET)
NET <- as.data.frame(NET) # KET analogue from NR3C1
RET <- as.data.frame(RET) # KET analogue from RUNX1
combET <- cbind(KET, NET, RET)

combET_means <- combET %>%
  rowwise() %>%
  mutate(mean_runtime = mean(c_across(everything())))

combET_standard_deviations <- combET %>%
  rowwise() %>%
  mutate(sd_runtime = sd(c_across(everything())))

means_and_sd <- cbind(combET_means$mean_runtime, combET_standard_deviations$sd_runtime)
rownames(means_and_sd) <- rownames(combET)
colnames(means_and_sd) <- c("Mean_Runtime", "SD_Runtime")

means_and_sd <- subset(means_and_sd, rownames(means_and_sd) != "CVadj")
rownames(means_and_sd)[rownames(means_and_sd) == "regreg"] <- "Regreg"


ggplot(data = as.data.frame(means_and_sd), aes(x = rownames(means_and_sd), y = Mean_Runtime)) +
  geom_bar(stat = "identity", fill = "steelblue", width = 0.6) +
  geom_errorbar(aes(ymin = Mean_Runtime - SD_Runtime, ymax = Mean_Runtime + SD_Runtime), width = 0.2,  position = position_dodge(0.9)) +
  labs(x = "Methods", y = "Mean Runtime (hours)", title = "", fill = "Method") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))


ggsave("../BM_runATAC/Plots/runtimes.png", width = width, height = height, dpi = dpi)
```


```{r}
runATAC(methods="diffTF",
        design=design,
        genome=genome,
        peakpath=peakpath,
        pmoi=pmoi,
        ...
        seqStyle=seqStyle,
        paired_arg=paired_arg,
        diffTFpath="/mnt/plger/fgerbaldo/DTFAIB/KLF1/diffTF/output/FINAL_OUTPUT/extension100/KLF1vsCTRL.all.summary.tsv.gz")
