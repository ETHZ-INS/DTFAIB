...
percentNR1H4 <- lapply(res, cofactors, FUN = function(x, cofactors){
    (sum(ifelse(x$padj<0.05 & row.names(x) %in% cofactors, 1, 0), na.rm = TRUE)/length(cofactors))*100
  })
percentNR1H4 <- as.data.frame(percentNR1H4)
rownames(percentNR1H4) = "NR1H4"
percentNR1H4
saveRDS(percentNR1H4, "../BM_ATACr/percentNR1H4")
```