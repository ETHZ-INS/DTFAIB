...
paired_arg = FALSE # paired-ended reads or not?
```
...
percentNR3C1 <- lapply(res, cofactors, FUN = function(x, cofactors){
    (sum(ifelse(x$padj<0.05 & row.names(x) %in% cofactors, 1, 0), na.rm = TRUE)/length(cofactors))*100
  })
percentNR3C1 <- as.data.frame(percentNR3C1)
rownames(percentNR3C1) = "NR3C1"
percentNR3C1
saveRDS(percentNR3C1, "../BM_ATACr/percentNR3C1")
```

```{r}
NBM <- readRDS("../BM_runATAC/BMNR3C1.rds")

runtime_list <- NBM$runtime

new_names <- c("CV", "CVadj", "decouple", "FGSEA", "ML", "StabSel", "msVIPER", "regreg", "ULM", "VIPER")
names(runtime_list) <- new_names

NET <- sapply(runtime_list, function(x) as.numeric(x["elapsed"]/3600))

barplot(NET, col = "blue", main = "Elapsed Runtime for Each Method",
