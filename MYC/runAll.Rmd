...
```
```{r}
MYC_BM <- readRDS("../BM_ATACr/BMMYC.rds")
MYC_BM$AUC_plots
MYC_BM$NotTrueTF
```
```{r}
...
percentMYC <- lapply(res, cofactors, FUN = function(x, cofactors){
    (sum(ifelse(x$padj<0.05 & row.names(x) %in% cofactors, 1, 0), na.rm = TRUE)/length(cofactors))*100
  })
percentMYC <- as.data.frame(percentMYC)
rownames(percentMYC) = "MYC"
percentMYC
saveRDS(percentMYC, "../BM_ATACr/percentMYC")
```
