...
percentRUNX1 <- lapply(res, cofactors, FUN = function(x, cofactors){
    (sum(ifelse(x$padj<0.05 & row.names(x) %in% cofactors, 1, 0), na.rm = TRUE)/length(cofactors))*100
  })
percentRUNX1 <- as.data.frame(percentRUNX1)
rownames(percentRUNX1) = "RUNX1"
percentRUNX1
saveRDS(percentRUNX1, "../BM_ATACr/percentRUNX1")
```

```{r}
RBM <- readRDS("../BM_runATAC/BMRUNX1.rds")

runtime_list <- RBM$runtime

new_names <- c("CV", "CVadj", "decouple", "FGSEA", "ML", "StabSel", "msVIPER", "regreg", "ULM", "VIPER")
names(runtime_list) <- new_names

RET <- sapply(runtime_list, function(x) as.numeric(x["elapsed"]/3600))

barplot(RET, col = "blue", main = "Elapsed Runtime for Each Method",
